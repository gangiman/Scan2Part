...
conv_to_region_type = {
    # kernel_size = [k, k, k, 1]
    'HYPERCUBE': ME.RegionType.HYPER_CUBE,
    'SPATIAL_HYPERCUBE': ME.RegionType.HYPER_CUBE,
    'SPATIO_TEMPORAL_HYPERCUBE': ME.RegionType.HYPER_CUBE,
    'HYPERCROSS': ME.RegionType.HYPER_CROSS,
    'SPATIAL_HYPERCROSS': ME.RegionType.HYPER_CROSS,
    'SPATIO_TEMPORAL_HYPERCROSS': ME.RegionType.HYPER_CROSS,
    # 'SPATIAL_HYPERCUBE_TEMPORAL_HYPERCROSS': ME.RegionType.HYPER_CUBE,
    'SPATIAL_HYPERCUBE_TEMPORAL_HYPERCROSS': ME.RegionType.CUSTOM,
    # 'SPATIAL_HYPERCUBE_TEMPORAL_HYPERCROSS': ME.RegionType.HYBRID # HYBRID was deprecated
}
...
#   0: 'HYPERCUBE',
#   1: 'SPATIAL_HYPERCUBE',
#   2: 'SPATIO_TEMPORAL_HYPERCUBE',
#   3: 'HYPERCROSS',
#   4: 'SPATIAL_HYPERCROSS',
#   5: 'SPATIO_TEMPORAL_HYPERCROSS',
#   6: 'SPATIAL_HYPERCUBE_TEMPORAL_HYPERCROSS ',
# }
int_to_region_type = {
...
def convert_region_type(region_type):
  ...
  return conv_to_region_type[int_to_region_type[region_type]]
...
def convert_conv_type(conv_type, kernel_size, D):
  ...
  if conv_type == 'SPATIAL_HYPERCUBE':
  ...
  elif conv_type == 'HYPERCROSS':
    # conv_type conversion already handled
    pass
  # elif conv_type == ConvType.SPATIO_TEMPORAL_HYPERCROSS:
  elif conv_type == 'SPATIO_TEMPORAL_HYPERCROSS':
    # conv_type conversion already handled
    assert D == 4
  # elif conv_type == ConvType.SPATIAL_HYPERCUBE_TEMPORAL_HYPERCROSS:
  elif conv_type == 'SPATIAL_HYPERCUBE_TEMPORAL_HYPERCROSS':
    ######### It doesn't work anymore in this version #########
...
def avg_unpool(kernel_size, stride=1, dilation=1, conv_type='HYPERCUBE', D=-1):
  assert D > 0, 'Dimension must be a positive integer'
  region_type, axis_types, kernel_size = convert_conv_type(conv_type, kernel_size, D)
  kernel_generator = ME.KernelGenerator(
      kernel_size, stride, dilation, region_type=region_type, axis_types=axis_types, dimension=D)
...
def sum_pool(kernel_size, stride=1, dilation=1, conv_type='HYPERCUBE', D=-1):
  assert D > 0, 'Dimension must be a positive integer'
  region_type, axis_types, kernel_size = convert_conv_type(conv_type, kernel_size, D)
  kernel_generator = ME.KernelGenerator(
      kernel_size, stride, dilation, region_type=region_type, axis_types=axis_types, dimension=D)
